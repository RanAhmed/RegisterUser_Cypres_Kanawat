...
describe('Register user and validate', () => {
  ...
  it('Validate Mobile Number Format', () => {
    ...
    registerobj.MobileTxr().type(env.MobileWrongFormat)
    registerobj.addressAlias().focus().clear().type(env.AddressAlias)
    registerobj.registerBtn().click();
    cy.get('.alert').should('contain.text', 'phone_mobile is invalid.')
  })
})